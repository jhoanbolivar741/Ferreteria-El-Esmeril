<x-app-layout>
    <div class="flex flex-col items-center justify-center mt-10">
        <div
            class="w-full max-w-sm p-4 bg-blue-50 border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
            <form class="space-y-6" action="{{route('unidades.update',[$unidad])}}" method="POST">
                @method('PUT')
                <h5 class="text-4xl font-medium text-gray-900 dark:text-white">Editar Unidad</h5>
                @csrf
                @if ($errors->any())
