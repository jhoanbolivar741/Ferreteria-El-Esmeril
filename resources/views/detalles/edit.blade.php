<x-app-layout>
    <div class="flex flex-col items-center justify-center mt-10">
        <div
            ...
            <form class="space-y-6" action="{{route('detalles.update',[$detalle])}}" method="POST">
                @method('PUT')
                <h5 class="text-xl font-medium text-gray-900 dark:text-white">Editar Detalle</h5>
                @csrf
                @if ($errors->any())
                ...
                <div>
                    <label for="ventas_id"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ventas ID</label>
                    <input type="number" name="ventas_id" id="ventas_id"
                        value="{{old('ventas_id',$detalle->ventas_id)}}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                    </input>
                </div>
                <div>
                    <label for="articulos_id"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Articulos ID</label>
                    <input type="number" name="articulos_id" id="articulos_id"
                        value="{{old('articulos_id',$detalle->articulos_id)}}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                        placeholder="100" />
                </div>
